...
public class CampsServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
    ...
    if(method != null){
      ...
      if(method.toLowerCase().equals("image")){
        //Get the node and transtype ids from the input paramaters
        ...
        if(node1 != null && node2 != null && transType != null){
          ...
          if(start != null && end == null){
            ...
            res.setContentType("image/png");//Since we are printing an image
            OutputStream out = res.getOutputStream();
            //write the generated circle onto the outputstream as a png
            ci.writeImage(out);
            out.close();
          }else{//Otherwise, we need to draw a path
            PathData pathData;
            if(start == null || end == null || transMethod == null){
              //If any one of the needed data objects is not found then we 
              //should return a blank image which is what an empty pathData represents
            ...
            res.setContentType("image/png");//Since we are printing an image
            OutputStream out = res.getOutputStream();
            //write the generated path onto the outputstream as a png
            pi.writeImage(out);
            out.close();
          }
        }else{
      ...
      }else{//The method is not image so we are returning text
